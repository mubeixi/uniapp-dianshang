...
var _default = { name: "App", // components: {
  ...
  //   pagetitle
  // },
  data: function data() {return { CartList: [], prodList: [], handleShow: true, checked: false, total_count: 0, total_price: 0, Users_ID: 'wkbq6nc2kc', User_ID: 3, prod_arg: { page: 1, pageSize: 4 }, hasMore: true // 是否还有产品
    };}, // 用户下拉
  onPullDownRefresh: function onPullDownRefresh() {}, // 上拉触底
  onReachBottom: function onReachBottom() {if (this.hasMore) {this.getProd();}}, created: function created() {this.getCart();this.getProd();}, methods: { handle: function handle() {this.handleShow = !this.handleShow;}, getCart: function getCart() {var _this = this;(0, _fetch.getCart)({ Users_ID: this.Users_ID, User_ID: this.User_ID, cart_key: 'CartList' }).then(function (res) {console.log(res);if (res.errorCode == 0) {_this.CartList = res.data.CartList;_this.total_count = res.data.total_count;_this.total_price = res.data.total_price;}}).catch(function (e) {return console.log(e);});}, getProd: function getProd() {var _this2 = this;this.prod_arg.Users_ID = this.Users_ID;var oldlist = this.prodList;(0, _fetch.getProd)(this.prod_arg).then(function (res) {console.log(res);if (res.errorCode == 0) {_this2.prodList = oldlist.concat(res.data);_this2.hasMore = res.totalCount / _this2.prod_arg.pageSize > _this2.prod_arg.page ? true : false;_this2.prod_arg.page += 1;}}).catch(function (e) {return console.log(e);});} }, computed: { manage: function manage() {return this.CartList.length == 0;} } };exports.default = _default;

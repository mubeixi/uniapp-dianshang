<template>
  <view :style="[getWrapStyle]" @click="toSearch" class="search wrap absolute" v-if="style.position==='absolute'">
    <view :class="{'round':search.config.type==='round'}" class="box">
      <icon :color="style.color" class="icon" size="16" type="search" />
      ...
      <view :style="{height:style.height+'px',lineHeight:style.height+'px',backgroundColor:style.inputBgColor,color:style.color}"
            class="input font14">
        {{placeholderText}}
      </view>
    </view>
  </view>
  <view :style="[getWrapStyle]" @click="toSearch" class="search wrap" v-else>
    <view :class="{'round':search.config.type==='round'}" class="box">
      <icon :color="style.color" class="icon" size="16" type="search" />
      <view :style="{height:style.height+'px',lineHeight:style.height+'px',backgroundColor:style.inputBgColor,color:style.color}"
            class="input font14">

<template>
  <div :style="{background:cube.style.bgColor}" class="cube wrap">
    <div :style="{width:fullW+'px'}" class="box">
      <div :style="[getWrapStyle()]" class="postion-wrap">
        <!--所有热区用绝对定位实现-->
        <div :data-idx="area.IDX" :key="aidx" :style="[getAreaStyle(area)]" @click="go(area)"
             class="active" v-for="(area,aidx) in cube.value.list">
          <div :style="{backgroundImage:'url('+domainFunc(area.bgimg)+')'}" class="mask"></div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    colH () {
      const res = uni.getSystemInfoSync()
      const Height = res.screenWidth * this.cube.config.row / this.cube.config.col

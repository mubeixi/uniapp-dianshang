<template>
  <div :style="{background:goods.style.bgColor,paddingLeft:goods.style.wrapmargin+'px',paddingRight:goods.style.wrapmargin+'px'}" class="goods wrap"
    ...
    <image :src="'/static/client/group_bg.png'|domain" class="bgimg"></image>
    <div :class="className" class="listbox">
      <ul class="list">
        <li :class="[idx%2==0?'even':'odd',goods.config.radius=='round'?'round':'',goods.config.showmode]" :key="idx"
            :style="[itemMarginObj(idx)]" @click="goProductDetail(item.Products_ID,item.is_pintuan)"
            class="item"

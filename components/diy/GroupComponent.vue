        ...
        <li  @click="goProductDetail(item.Products_ID,item.is_pintuan)" v-for="(item,idx) in goodsList" :key="idx" class="item" :class="[idx%2==0?'even':'odd',goods.config.radius=='round'?'round':'',goods.config.showmode]" :style="[itemMarginObj(idx)]">
          ...
          <div class="info" :style="{width:goods.config.style==2?itemw:''}" :class="{empyInfo:isEmpeyInfo}">
            <div class="left">
              ...
              <div v-if="goods.config.style!=1" v-show="goods.config.attr.price.show" class="price"><span class="graytext2 font12">拼团价 </span><span class="sign">￥</span><span style="font-weight: 600">{{item.pintuan_pricex}}</span><span class="graytext2 market-price font12"> ￥{{item.Products_PriceX}} </span>
              </div>
            </div>
            ...
            <div class="bottom-box">
              <div v-if="goods.config.style==1" v-show="goods.config.attr.price.show" class="price"><span class="graytext2 font12">拼团价 </span><span class="sign">￥</span><span style="font-weight: 600">{{item.pintuan_pricex}}</span><span class="graytext2 market-price font12"> ￥{{item.Products_PriceX}} </span></div>

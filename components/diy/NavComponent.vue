<template>
  <view :style="{background:style.bgColor}" class="nav wrap">
    <view class="box style1" v-if="nav.value.list">
      <view :class="displayStyle" :style="{display:nav.value.list.length<6?'flex':'block'}" class="list">
        <view :key="idx" :style="{flex:nav.value.list.length<6?'1':''}" @click="go(item)" class="item"
              v-for="(item,idx) in nav.value.list">

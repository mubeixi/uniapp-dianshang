...
function pro_format(pro_list, front_url) {
    ...
    for (var i in pro_list) {
        ...
        pro_list[i]['is_attr'] = pro_list[i]['skujosn'] != '' && pro_list[i]['skuvaljosn'] != '' ? true : false;
        //攻略处理
...
function activity_format(activity_list) {
    ...
    for (var i in activity_list) {
        let tag = activity_list[i].activity_tag;
        if (tag == 'newuser') { //新用户
        ...
        if (tag == 'manjian') { //满减
            let intro = activity_list[i].activity_intro = typeof activity_list[i].activity_intro != 'object' ? JSON.parse(activity_list[i].activity_intro) : activity_list[i].activity_intro;
            let desc = '';
            for (var j in intro) {

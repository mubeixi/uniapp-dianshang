...
function roomListFormat(front_url, roomList) {
	for (var i in roomList) {
		...
		for (var j in roomList[i].img_path) {
			roomList[i].img_path[j] = roomList[i].img_path[j].indexOf() > -1 ? roomList[i].img_path[j] : front_url + roomList[i]
				.img_path[j]
		}
	}
	return roomList
...
function formatOrder(front_url, orderlist) {
	for (var i in orderlist) {
		var CartList = orderlist[i].Order_CartList = orderlist[i].Order_CartList !== '' && typeof orderlist[i].Order_CartList !==
			'object' ? JSON.parse(orderlist[i].Order_CartList) : orderlist[i].Order_CartList
		if (CartList !== '') {
		...
		// 配送费
		orderlist[i].Order_Shipping = orderlist[i].Order_Shipping && orderlist[i].Order_Shipping !== '' && typeof orderlist[i]
			.Order_Shipping !== 'object' ? JSON.parse(orderlist[i].Order_Shipping) : orderlist[i].Order_Shipping
		// 订单时间
		...
		// 状态颜色
		orderlist[i].status_color = '1,2,3,4'.indexOf(orderlist[i].Order_Status) > -1 || (orderlist[i].Order_Status === 5 &&
			orderlist[i].Is_Backup === 1) ? '#0ca400' : ('0,6'.indexOf(orderlist[i].Order_Status) > -1 ? '#ff4901' : '#333')

		// 商家头像处理
		orderlist[i].Biz_Logo = !orderlist[i].Biz_Logo ? '/images/moren.png' : (orderlist[i].Biz_Logo.indexOf('http') > -1 ||
			orderlist[i].Biz_Logo.indexOf('/uploadfiles/') === -1 ? orderlist[i].Biz_Logo : front_url + orderlist[i].Biz_Logo)

		// 核销码
		orderlist[i].consumption_qrcode = !orderlist[i].consumption_qrcode ? '' : (orderlist[i].consumption_qrcode.indexOf(
			'http') > -1 ? orderlist[i].consumption_qrcode : front_url + orderlist[i].consumption_qrcode)
	}
	return orderlist
}
...
function cate_format(cate_list, front_url) {
	for (var i in cate_list) {
		cate_list[i].cate_img = cate_list[i].cate_img === '' ? '/images/none.png' : (cate_list[i].cate_img.indexOf('http') >
			-1 ? cate_list[i].cate_img : front_url + cate_list[i].cate_img)
	}
	return cate_list
}
...
function activity_format(activity_list) {
	...
	for (var i in activity_list) {
		...
		if (tag === 'newuser') { // 新用户
			...
			if (T.isPre('en-us')) {
				activity_list[i].desc = `${activity_list[i].activity_intro} yuan discount for new users`
			}
		...
		if (tag === 'sendfree') { // 免费赠送
			...
			if (T.isPre('en-us')) {
				activity_list[i].desc = `Free delivery fee for ${activity_list[i].activity_intro} yuan`
			}
			activity_list[i].desc =
				list.push(activity_list[i])
		}
		if (tag === 'manjian') { // 满减
			const intro = activity_list[i].activity_intro = typeof activity_list[i].activity_intro !== 'object' ? JSON.parse(
				activity_list[i].activity_intro) : activity_list[i].activity_intro
			let desc = ''
			for (var j in intro) {
				if (T.isPre('zh-cn')) {
					desc += '满' + intro[j].full + '元减' + intro[j].cut + '元'
				}
				if (T.isPre('en-us')) {
					desc += `${intro[j].full} yuan off if you spend ${intro[j].cut} yuan`
				}
				desc +=  (j % 2 === 1 ? '\n' : '　')
			}

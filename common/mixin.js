 ...
 * 统计的混合类
 */
import { getJsSign, getSystemConf, sendAnalysisData, upUserLog } from './fetch'
...
export const WX_JSSDK_INIT = (vm, jsApiListList) => new Promise((resolve, reject) => {
  if (!isWeiXin()) reject(new Error('请在微信中打开'))
...
export const pageMixin = {
  ...
  methods: {
    async default_init_func (options) {
      ...
      const store_id = options.store_id || GetQueryByString(location.href, 'store_id')
      if (store_id) {
      ...
      if (users_id) {
        // 暂存一下，会用到
        const old_users_id = ls.get('users_id')
      ...
      // 根据配置决定是否刷新配置
      const initData = await this.getInitData(this.refreshInit)

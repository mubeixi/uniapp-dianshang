	...
	function KDNSearchResultService(options) {
		...
		this.validateData = function() {
			...
			if (this.param.color.substring(0, 3) != "rgb" || this.param.color.substring(0, 3) != "RGB") this.param.color =
				"rgb(46,114,251)";
			return true
		};
		this.excuteProcess = function() {
			window.location.href = kdnOptions.baseUrl + "MSearchResult.aspx?expCode=" + this.param.expCode + "&expNo=" + this.param
				.expNo + "&backUrl=" + window.location.href + "&color=" + this.param.color
		}
	}
	...
	function KDNPCSearchResultService(options) {
		...
		this.validateData = function() {
			if (!utilityService.hasProperty(this.param, "expCode") || !utilityService.hasProperty(this.param, "expNo") || !
				utilityService.hasProperty(this.param, "container")) return false;
			return true
		};
		this.excuteProcess = function() {
			...
			if (_showType == "normal") {
				...
				var myhtml =
					"<div class='kdnlogin-title'>即时查询结果<div class='kdnlogin-close'>关闭</div></div><div class='kdnlogin-content'><iframe width='900' height='550' frameborder='0' scrolling='auto' src='" +
					iframeSrc + "'> </iframe></div>";
	...
	function KDNSearchTrackService(options) {
		...
		this.excuteProcess = function() {
			window.location.href = kdnOptions.baseUrl + "MSearchTrack.aspx?backUrl=" + window.location.href + "&sortType=" +
				this.param.sortType + "&color=" + this.param.color
		}
	}
	...
	function KDNPCSearchTrackService(options) {
		...
		this.excuteProcess = function() {
			...
			var iframeHtml = "<iframe frameborder='0' width='1012' height='780' scrolling='auto' src='" + kdnOptions.baseUrl +
				"SearchTrack.aspx'></iframe>";
	...
	function KDNOnlineOrderService(options) {
		...
		this.excuteProcess = function() {
			window.location.href = kdnOptions.baseUrl + "MOnlineOrder.aspx?backUrl=" + window.location.href + "&sortType=" +
				this.param.sortType + "&color=" + this.param.color
		}
	}
	...
	function KDNPCOnlineOrderService(options) {
		...
		this.excuteProcess = function() {
			...
			targetContainer.style.margin = "0px auto";
			var iframeHtml = "<iframe frameborder='0' width='1012' height='700' scrolling='auto' src='" + kdnOptions.baseUrl +
				"OnlineOrder.aspx'></iframe>";
			targetContainer.innerHTML = iframeHtml
		}
	}

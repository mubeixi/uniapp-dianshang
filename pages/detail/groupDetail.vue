	...
	<div @click="commonClick" class="wrap" style="position:relative;background-color:#f8f8f8">
		...
		<div class="flex-message flex" v-if="initData.Substribe&&userInfo.is_subscribe==0&&showWxChatSwitch==1">
			    ...
			    <view class="flex flex-vertical-center">
						...
						<image class="codeDel" src="/static/no/detailX-no.png"></image>
					</view>
					
		</div>  
		<div class="flex-message-occupy"  v-if="initData.Substribe&&userInfo.is_subscribe==0&&showWxChatSwitch==1"></div>
		
		<div class="top"  :class="{topIndex:initData.Substribe&&userInfo.is_subscribe==0&&showWxChatSwitch==1}">
		...
		<view class="uni-padding-wrap" style="background: #f2f2f2;">
			<view class="page-section swiper">
				<view class="page-section-spacing" v-if="(product.video_url && showVideo)">
					<video :poster="product.cover_url?product.cover_url:''" :src="product.video_url|domain" @pause="pause" @play="play"
					 bindfullscreenchange="changeHiddenBtns" class="video" controls id="myVideo1" ref="video1" show-center-play-btn>
					</video>
		...
		<div class="section1">
			...
			<div class="rightss">
				<div class="countdown" v-if="$p('zh-cn')">
				...
				<div class="haha" v-if="$p('en-us')">
					{{product.Products_Sales}} groups have been grouped {{product.pintuan_people}} people group
				</div>
			</div>
		</div>
		<!-- 领券 -->
		...
		<div class="section3" v-if="product.Products_Promise && product.Products_Promise.length > 0">
			<span :key="index" class="span" v-for="(item,index) in product.Products_Promise" v-if="item.name">
		...
		<popupLayer :direction="'top'" ref="popupLayer">
			...
			<scroll-view @scrolltolower="goNextPage" class="ticks" scroll-y=true v-if="type=='ticks'">
				...
				<div :key="i" :style="{backgroundImage: 'url('+$fun.domainFn('/static/client/detail/yhq.png')+')'}" class="t_content"
		...
		<popupLayer :direction="'top'" ref="cartPopu">
			<div class="cartSku">
				<div class="cartTop">
					<image :src="skuImg?skuImg+'-r640':product.Products_JSON.ImgPath[0]+'-r640'" @click="yulanDetail" class="image"></image>
					<div class="cartTitle">
				...
				<div class="cartCenter">
					<div :key="i" class="cartAttr" v-for="(item,i) of product.skujosn">
						...
						<div class="skuValue">
							<div :class="check_attr[i]==index?'skuCheck':''" :key="index" @click="selectAttr(index,i)" class="skuview" v-for="(mbx,index) of item">{{mbx}}
							</div>
						</div>
					</div>
				</div>
		...
		<div class="fixed">
			...
			<div class="rightss">
				<form @click="myPay" class="form" report-submit>

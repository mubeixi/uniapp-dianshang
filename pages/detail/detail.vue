<template>
	<div @click="commonClick" style="position:relative;" class="wrap">
		...
		<view class="uni-padding-wrap" style="background: #f2f2f2;">
			<view class="page-section swiper">
				<view class="page-section-spacing" v-if="(product.video_url && showVideo)">
					<video class="video" @play="play" @pause="pause" :src="product.video_url" bindfullscreenchange="changeHiddenBtns"
					 :poster="product.cover_url?product.cover_url:''" id="myVideo1" ref="video1" show-center-play-btn controls>
						<!-- #ifdef APP-PLUS -->
		...
		<div class="comment bgwhite">
			<div class="c_title">
				...
				<div class="right" @click="gotoComments">查看全部
					<image style="width: 19rpx;height: 30rpx;margin-left: 20rpx;" :src="'/static/client/detail/right.png'|domain" alt="" />
				</div>
			</div>
		...
		<bottom @cartHandle="addCart" @directHandle="directBuy" @goGet="lingqu" @collect="collect" :collected="isCollected"
		 :recieve="recieve" :isVirtual="isVirtual" :canSubmit="canSubmit"></bottom>
				...
				<div class="cartCenter">
					<div class="cartAttr" v-for="(item,i) of product.skujosn_new" :key="i">
						...
						<div class="skuValue" v-if="gift == 0">
							<div class="skuview" :class="check_attr[item.sku]==index?'skuCheck':''" @click="selectAttr(index,item.sku)"
							 v-for="(mbx,index) of item.val" :key="index">{{mbx}}</div>
						</div>

			...
			<div class="right">
				...
				<div class="bottom">
					<div class="price">
						...
						<span v-if="$p('zh-cn')">拼团省{{product.Products_PriceX.pintuan_pricex}}</span>
						<span v-if="$p('en-us')">Save {{product.Products_PriceX.pintuan_pricex}} yuan by joining the group</span>
					</div>
			...
			stampCount() {
				...
				if (this.product && this.product.pintuan_end_time) {
					...
					const computedStamp = getGroupCountdown({
						end_timeStamp: parseInt(this.addtime) + 24 * 60 * 60
					})
					if (computedStamp) {
						rt = computedStamp
					} else {

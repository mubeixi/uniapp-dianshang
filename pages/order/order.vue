<template>
  <div @click="commonClick" class="haihong">
    ...
    <div :key="index" @click="goDetail(item)" class="order" v-for="(item,index) of data">
      <template v-if="item.prod_list.length>0">
        ...
        <div class="bizinfo">
          <img :src="item.ShopLogo|domain" alt="" class="img">
        ...
        <div class="btn-group" v-if="item.Order_Status==1">
          <span @click.stop="cancelOrder(item.prod_list,index)">取消订单</span>
          <span @click.stop="goPay(item)" class="active">立即付款</span>
        </div>
        <div class="btn-group" v-else-if="item.Order_Status==2&&item.Order_Type != 'gift'">
          <span @click.stop="goPay(item)" class="active">申请退款</span>
        </div>
        <div class="btn-group" v-else-if="item.Order_Status==3">
          <div @click.stop="openExtendReceiptFn(item)" class="extend_receipt" v-if="item.allow_extend_receipt">
            ...
            <div @click.stop="extendReceiptFn(item)" class="tooltip" v-if="item.extend">确认延迟</div>
          </div>
          ...
          <span @click.stop="confirmOrder(item,index)" class="active">确认收货</span>
          <!-- @click="goPay(item)"跳转退款 -->
        </div>
        <div class="btn-group" v-else-if="item.Order_Status==4 && item.Is_Commit == 0 && item.Is_Backup == 0">
          <span @click.stop="goPay(item)" class="active">立即评价</span>
        </div>
      </template>
      <template v-if="item.prod_list.length>0">
        <div style="background-color: #F3F3F3;height: 20rpx;width: 100%;position: absolute;left: 0rpx;"></div>
...
import { cancelOrder, confirmOrder, delOrder, extendOrderConfirm, getOrder, getOrderNum } from '../../common/fetch.js'

  ...
  <view @click="commonClick" class="wrap">
      ...
      <view class="storeAddressRight">
        <view class="storeName">
          ...
          <view class="storeKm">
            ...
            <image :src="'/static/client/person/right.png' | domain" class="imgHeight"></image>
          </view>
        </view>
    ...
    <view class="storeCate">
      <view class="storeCateLine">
        ...
        <scroll-view class="storeScroll" scroll-x="true">
          <block :key="index" v-for="(item,index) of cateList">
            <!-- skuSelect -->
            <view :class="indexFirst==index?'skuSelect':''" @click="selctFirst(index)" class="skuClass ">
              {{item.Category_Name}}
            </view>
          </block>
        </scroll-view>
      ...
      <view class="storeCateLine">
        ...
        <scroll-view class="storeScroll" scroll-x="true" v-if="indexFirst>-1">
          <block :key="ind" v-for="(it,ind) of cateList[indexFirst].child">
            <!-- skuSelect -->
            <view :class="indexSecond==ind?'skuSelect':''" @click="selctSecond(ind)" class="skuClass ">
              {{it.Category_Name}}
            </view>
          </block>
        </scroll-view>
      ...
      <view class="storeCateLast">
        ...
        <view class="selectSku" v-if="indexSecond>=0&&indexFirst>=0">
          {{cateList[indexFirst].child[indexSecond].Category_Name}}
          <image @click="delSecond" class="delSku" src="/static/skuDel.png"></image>
        </view>
      </view>
    </view>
    ...
    <view class="prolist">
      <view @click="add(item)" class="pro" v-for="item in prolist">
        ...
        <view class="pro-price">
          ...
          <view class="old-price" v-if="item.Products_PriceY!=item.Products_PriceX">
            <text class="money-icon">￥</text>
    ...
    <view style="height:90rpx;" v-if="total_cart_count > 0">
      <view :style="{'z-index': zIndex}" class="check">
        ...
        <view @click="submit" class="submit">提交进货单</view>
      </view>
    </view>
    <!--  遮罩层	-->
    ...
    <view class="sku-pop" v-if="showSku">
      ...
      <view class="sku-content">
        <view class="skulist" v-for="item in prosku.skujosn_new">
          ...
          <view class="sku-item">
            <view :class="[check_attr[item.sku]==index?'active':'','sku']" @click="selectAttr(index,item.sku)"
                  v-for="(attr,index) of item.val">{{attr}}
            </view>
          </view>
        </view>
        <view class="skulist">
          ...
          <view class="sku-item">
            ...
            <input @blur="setCount" class="pro-num" v-model="postData.qty" />
            <view @click="plus" class="handle">
              <view class="handle-bg">+</view>
            </view>
          </view>
        </view>
        <view class="sku-btns">
          ...
          <view :class="submit_flag?'':'disabled'" @click="confirm" class="confirm btn">确定</view>
        </view>
      </view>
    </view>
    <!--	明细	-->
            ...
            <view class="product">
              ...
              <view class="proMsg">
                <view class="proName">
                  ...
                  <image @click="del(pro_id,attr_id)" class="del" src="/static/procurement/del.png"></image>
                </view>
                <view class="attrInfo">
                ...
                <view class="proPrice">
                  <view class="newPrice">￥
                    <text class="number">{{attr.ProductsPriceX}}</text>
                  </view>
                  <view class="oldPrice">￥{{attr.ProductsPriceY}}</view>
                  <view class="amount">
                    <view @click="reduce(pro_id,attr_id)" class="icon">-</view>
                    <view class="num">{{attr.Qty}}</view>
                    <view @click="increase(pro_id,attr_id)" class="icon">+</view>
                  </view>
                </view>
              </view>
            </view>
  ...
  methods: {
    ...
    getProlist (index) {
      ...
      getPifaStoreProd(data, { tip: 'loading' }).then(res => {
        ...
        this.get_more = !!(res.totalCount / this.prolist.length)
      }).catch(e => {
        setTimeout(function () {

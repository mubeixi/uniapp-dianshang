<template>
  <view @click="commonClick">
    ...
    <popup-layer :direction="'top'" ref="popupLayer">
      <div class="iMbx">
              <div :key="index" @click="chooseType(index)" class="c_method flex flex-vertical-c "  style="padding-left: 200rpx" v-for="(item,index) in initData.pay_arr">
    ...
    async disBuy () {
      ...
      if (this.pro.dis_config.Level_Form) {
        ...
        if (this.select_lists.length > 0) {
          for (const item of this.select_lists) {
            const arr = {}
        ...
        if (this.type == 'edit') {
          ...
          if (!((JSON.stringify(dislist.apply_order) == '{}'))) {
            ...
            for (const ite in myInfo) {
              ...
              for (const select in this.select_lists) {
          ...
          // 处理自定义  select  和text
          const dislist = this.pro.dis_config

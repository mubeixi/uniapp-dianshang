    ...
    <div :key="index" class="pro-list" v-for="(item,index) in collect_list">
      ...
      <div @click="goDetail(item)" class="pro">
        ...
        <div class="pro-msg">
          ...
          <div class="collection"><span>{{item.favourite_count}}</span>人{{$t(1055)}}</div>
          <div class="btn">
                      <span class="span">
            ...
            <form @submit="buy" report-submit>
              <button class="button" formType="submit">{{$t(1056)}}</button>
            </form>
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    cancelCollection () {
      ...
      cancelCollection({
        ...
        this.collect_list = []
        this.rightText = this.rightClicked ? T._(1055) : T._(1056)
      }).catch(e => {
      })
    },
    // 单选
    ...
    rightHandle () {
      ...
      this.rightText = this.rightClicked ? T._(1057) : T._(1058)
    },
    // 获取收藏列表
    getFavouritePro () {
      getFavouritePro({

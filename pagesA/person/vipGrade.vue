  ...
  <view @click="commonClick" class="all">
    <view class="top">
      <swiper :autoplay="false" :current="inds" :duration="1000" :indicator-dots="false" @change="change"
              class="center">
        <swiper-item :key="index" :style="pro.levels.length==1?'margin-left:43rpx;':''" class="vipFir"
                     v-for="(item,index) of pro.levels">
    ...
    <scroll-view class="mbxa" scroll-x="true">
      <view :key="mx" class="floats" v-for="(mb,mx) of tequan.basic">
        <view class="floatm">
          <view class="imgs">
            <image :src="mb.img_url" class="image"></image>
          </view>
          {{mb.name}}
        </view>
      </view>
